...
class DailyDataService:
    ...
    def get_latest_reading(self, station_id: str) -> Optional[Dict[str, Any]]:
        """
        Get the most recent daily mean reading for a station.
        
        Args:
            station_id: Environment Canada station identifier
            
        Returns:
            Dict with date and values, or None if not found
        """
        try:
            params = {
                'STATION_NUMBER': station_id,
            ...
            if features:
                props = features[0].get('properties', {})
                daily_mean = DailyMean.from_api_response(props)
                
                if daily_mean and daily_mean.date:
                    result = {'date': daily_mean.date}
                    if daily_mean.discharge is not None:
                        result['discharge'] = daily_mean.discharge
                    if daily_mean.level is not None:
                        result['level'] = daily_mean.level
                    return result
