...
# Configuration
CONFIG = {
    'hours_to_fetch': 336,  # Fetch last 14 days (14 * 24 = 336 hours)
    'log_level': 'INFO',
}
...
def update_station_realtime(
    ...
    try:
        # Get station status
        status = data_service.get_station_status(station_id)
        ...
        if not dry_run:
            ...
            all_readings = data_service.fetch_latest_readings(station_id, hours=hours)
...
def main():
    ...
    try:
        ...
        # Update all stations
        logger.info(f"\n3. Updating {len(ec_stations)} stations with real-time data...")
        
        all_stats = []
        
        for i, station_id in enumerate(sorted(ec_stations), 1):
            logger.info(f"\n[{i}/{len(ec_stations)}] {station_id}")
            
            stats = update_station_realtime(
                station_id,
                data_service,
                manager,
                args.hours,
